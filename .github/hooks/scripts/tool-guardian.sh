...
# Tool Guardian Hook (fixed for GitHub Copilot hook spec)
# Blocks dangerous tool operations before the Copilot coding agent executes them.
#
# Upstream: github/awesome-copilot — toolInput → toolArgs fix applied
#
# Environment variables:
#   GUARD_MODE           - "warn" (log only) or "block" (deny on threats) (default: warn)
#   SKIP_TOOL_GUARD      - "true" to disable entirely (default: unset)
#   TOOL_GUARD_LOG_DIR   - Directory for guard logs (default: .github/logs/copilot)
...
# Copilot spec: { "toolName": "bash", "toolArgs": "{\"command\":\"...\"}" }
INPUT=$(cat)

MODE="${GUARD_MODE:-warn}"
LOG_DIR="${TOOL_GUARD_LOG_DIR:-.github/logs/copilot}"
TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/tool-guardian.log"

# ---------------------------------------------------------------------------
# Extract toolName and command from Copilot spec fields
# toolArgs is a JSON string — parse .command out of it
# ---------------------------------------------------------------------------
TOOL_NAME=""
TOOL_ARGS_RAW=""
TOOL_COMMAND=""
...
if command -v jq &>/dev/null; then
  ...
  # toolArgs is itself a JSON string, parse .command from it
  if [[ -n "$TOOL_ARGS_RAW" ]]; then
    TOOL_COMMAND=$(printf '%s' "$TOOL_ARGS_RAW" | jq -r '.command // empty' 2>/dev/null || echo "")
  fi
fi
...
# Combine toolName + command for pattern matching
COMBINED="${TOOL_NAME} ${TOOL_COMMAND}"
...
if [[ $THREAT_COUNT -gt 0 ]]; then
  ...
  if [[ "$MODE" == "block" ]]; then
    ...
    jq -n --arg reason "Dangerous command detected: $(printf '%s\n' "${THREATS[@]}" | head -1 | cut -f1)" \
      '{permissionDecision: "deny", permissionDecisionReason: $reason}'
    exit 0
  else
    echo "⚠️  Threat logged (warn mode). Set GUARD_MODE=block to block dangerous operations."
  fi
else
