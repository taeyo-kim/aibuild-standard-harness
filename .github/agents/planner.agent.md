...
tasks:
  - id: string
    ...
    agent: string # researcher | implementer | browser-tester | devops | reviewer | documentation-writer | debugger | critic | code-simplifier | designer
...
|:----------|:--------------|:------------|
| Code implementation | implementer | Feature code, bug fixes, refactoring |
| Research/analysis | researcher | Exploration, pattern finding, investigating |
| Planning/strategy | planner | Creating plans, DAGs, roadmaps |
| UI/UX work | designer | Layouts, themes, components, design systems |
| Refactoring | code-simplifier | Dead code, complexity reduction, cleanup |
| Bug diagnosis | debugger | Root cause analysis (if requested), NOT for implementation |
| Code review | reviewer | Security, compliance, quality checks |
| Browser testing | browser-tester | E2E, UI testing, accessibility |
| DevOps/deployment | devops | Infrastructure, CI/CD, containers |
| Documentation | documentation-writer | Docs, READMEs, walkthroughs |
| Critical review | critic | Challenge assumptions, edge cases |
| Complex project | All 11 agents | Orchestrator selects based on task type |

**Special assignment rules:**
- UI/Component tasks: implementer for implementation, designer for design review AFTER
- Security tasks: Always assign reviewer with review_security_sensitive=true
- Refactoring tasks: Can assign code-simplifier instead of implementer
- Debug tasks: debugger diagnoses but does NOT fix (implementer does the fix)
- Complex waves: Plan for critic after wave completion (complex only)
